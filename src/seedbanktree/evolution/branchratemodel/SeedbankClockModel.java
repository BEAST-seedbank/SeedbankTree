...
public class SeedbankClockModel extends BranchRateModel.Base {
	...
	final public Input<Function> alphaInput = new Input<>("alpha", 
					"the scaling factor applied to the substitution model of dormant branches.", Validate.REQUIRED);
    ...
    private double[] rates; // The computed rates

	@Override
	public void initAndValidate() {
		
		alpha = alphaInput.get();
		if (alpha instanceof RealParameter) {
			RealParameter dS = (RealParameter) alpha;
	...
	public void calculateRates(Node node) {
		...
		if (node.isRoot()) {
			...
			double newRate = 0;
			newRate += (dormantBranchLength / branchLength) * alpha.getArrayValue();
