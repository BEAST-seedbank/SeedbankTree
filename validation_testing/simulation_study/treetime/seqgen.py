...
class SeqGen(TreeAnc):
    ...
    def __init__(self, L, alpha=None, gtr_d=None, *args, **kwargs):
    ...
    def evolve_sb(self, root_seq=None):
        """Evolve a root sequences along a tree. If no root sequences
        is provided, one will be sampled from the equilibrium
        probabilities of the GTR model
        ...
        for n in self.tree.get_nonterminals(order='preorder'):
            ...
            # print(profile_p)
            for c in n:
                if ("dormant" in c.comment and self.gtr_d):
                    profile = self.gtr_d.evolve(profile_p, c.branch_length)
                    # print(n.name, c.name, "dormant evolution", profile)
                else:
                    profile = self.gtr.evolve(profile_p, c.branch_length)
                    # print(n.name, c.name, "active evolution", profile)
                c.ancestral_sequence = self.sample_from_profile(profile)
                # print("selected sequence", c.ancestral_sequence)
