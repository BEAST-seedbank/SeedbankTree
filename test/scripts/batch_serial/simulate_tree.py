...
def newick(id, types, time_vector, anc, dec_1, dec_2):
    ...
    newickStr = ""
    if dec_1[id] != -1:
        newickStr += "(" + newick(dec_1[id], types, time_vector, anc, dec_1, dec_2)
        if dec_2[id] != -1:
            newickStr += "," + newick(dec_2[id], types, time_vector, anc, dec_1, dec_2)
        newickStr += ")"
    newickStr += str(id) + "[&type=\"" + type + "\"]:" + str(length)

    return newickStr

def simulate_tree(n_leaves, leaf_types, leaf_times, transition_rate, relative_seedbank_size, theta, log=None, seed=None):
    if not seed:
