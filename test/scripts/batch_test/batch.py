        ...
        for j in range(1, batch_size+1):
            ...
            if isochronous:
                ...
                for k, [lo, hi] in enumerate(time_ranges):
                    ...
                    # types = np.append(types, np.ones(sample_sizes_1[k]))
                    # types = np.append(types, np.zeros(sample_sizes_0[k]))
            ...
            with open(f"./{folder}/{i}/{j}/{j}.xml") as f:
                ...
                # parameter_scalers_list = []
                # for param, _bool in zip([c, K, theta, alpha, mu], [c_constant, K_constant, theta_constant, alpha_constant, mu_constant]):
                #     if _bool:
