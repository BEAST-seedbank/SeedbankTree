...
def likelihood(ordering, seq_list):
    ...
    P_params_active = GTR_model(pi, GTR_params_active)
    P_params_dorm = GTR_model(pi, GTR_params_dorm)
    LL = 0
...
def create_test_tree_topology(type=1):
    ...
    def dormant_active(a, len1=1, len2=1, len3=1):
        '''
        :param nodes: list of nodes
        :param a: node to go dormant and active
        :return: list of nodes with two new nodes along branch connected by dormancy
        '''
        next_num = len(nodes)
        parent1 = Node(next_num, False, 1)
        parent1.childs.append(nodes[a])
        parent1.branch_length = len2
        nodes[a].parent = parent1
        nodes[a].branch_length = len1
        next_num += 1
        parent2 = Node(next_num, True, 1)
        parent2.childs.append(parent1)
        parent2.branch_length = len3
        parent1.parent = parent2
        nodes.append(parent1)
